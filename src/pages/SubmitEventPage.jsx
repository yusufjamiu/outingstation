// COMPLETE SubmitEventPage.jsx - WITH CLOUDINARY UPLOAD + REFERRAL CODE + VENDOR
...
const SubmitEventPage = () => {
  ...
  const handleVendorImageChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      if (file.size > 10 * 1024 * 1024) {
        setErrors(prev => ({ ...prev, vendorImage: 'Image must be less than 10MB' }));
        return;
      }
      setFormData(prev => ({ ...prev, vendorImage: file }));
      const reader = new FileReader();
      reader.onloadend = () => setVendorImagePreview(reader.result);
      reader.readAsDataURL(file);
    }
  };
  ...
  const validate = () => {
    ...
    if (isVendor) {
      ...
      if (!formData.vendorImage) e.vendorImage = 'Shop photo required';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isVendor) {
        let imageUrl = 'https://via.placeholder.com/400x400/0891b2/ffffff?text=Shop';

        if (formData.vendorImage) {
          try {
            imageUrl = await uploadToCloudinary(formData.vendorImage, 'vendors');
          } catch (err) {
      ...
      // ✅ Handle event/place submission (existing logic)
      let imageUrl = 'https://via.placeholder.com/800x400/0891b2/ffffff?text=Event+Image';

      if (formData.eventImage) {
        try {
          const uploadPromise = uploadToCloudinary(formData.eventImage, 'events');
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-cyan-50 py-12 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-3xl shadow-2xl p-8 md:p-12">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                ...
                <p className="text-xs text-green-600 mt-2 font-medium flex items-center gap-1">
                  <span>💰</span>
                  <span>Earn ₦100 credits when your listing is approved!</span>
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {[
                  ...
                  <button
                    key={item.value}
                    type="button"
                    onClick={() => setFormData(prev => ({ ...prev, listingType: item.value }))}
                  ...
                  >
                    ...
                    <div className={`text-sm ${formData.listingType === item.value ? 'text-cyan-100' : 'text-gray-500'}`}>
                      {item.desc}
                    </div>
                  </button>
                ))}
              ...
              <div className="space-y-6">
                <div className="bg-emerald-50 border-2 border-emerald-200 rounded-2xl p-6">
                  ...
                  <p className="text-sm text-gray-600">
                    Register your campus shop — students will find you through the OutingStation app and contact you via WhatsApp.
                  </p>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <select
                      name="vendorCategory" value={formData.vendorCategory} onChange={handleChange}
                  ...
                  <div>
                    ...
                    <select
                      name="vendorUniversity" value={formData.vendorUniversity} onChange={handleChange}
                ...
                <div>
                  ...
                  <textarea
                    name="vendorDescription" value={formData.vendorDescription} onChange={handleChange} rows={3}
                ...
                {/* Shop Photo */}
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Shop Photo <span className="text-red-500">*</span></label>
                  <input
                    type="file" accept="image/*" onChange={handleVendorImageChange}
                    className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-cyan-500 focus:outline-none transition file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-cyan-50 file:text-cyan-700 hover:file:bg-cyan-100"
                  />
                  {errors.vendorImage && <p className="text-red-500 text-sm mt-1">{errors.vendorImage}</p>}
                  ...
                  {vendorImagePreview && (
                    <div className="mt-3 rounded-2xl overflow-hidden border-2 border-gray-200">
                      <img src={vendorImagePreview} alt="Shop preview" className="w-full h-48 object-cover" />
                    </div>
                  )}
                </div>
              </div>
          ...
          {/* SECTION 4: Location (Events/Places only) */}
          {isPhysical && (
            <div className="mb-10">
              <div className="flex items-center mb-6">
          ...
          {/* SECTION 6: Ticketing (Events/Places only) */}
          {!isVendor && (
            <div className="mb-10">
              <div className="flex items-center mb-6">
          ...
          {/* SECTION 7: Image (Events/Places only) */}
          {!isVendor && (
            <div className="mb-10">
              <div className="flex items-center mb-6">
          ...
          {/* SECTION 8: Additional Info (Events/Places only) */}
          {!isVendor && (
            <div className="mb-10">
              <div className="flex items-center mb-6">
              ...
              <div className="p-6 space-y-6 text-gray-700">
                ...
                <section>
                  ...
                  <p className="text-sm">
                    ...
                    <a href="mailto:admin@outingstation.com" className="text-cyan-600 hover:underline font-medium">
