...
export default function AmbassadorEventForm() {
  ...
  const calculateTotal = () => parseInt(ticketPrice || 0) + calculateServiceFee() + calculatePaystackFee();
  ...
  useEffect(() => {
    if (isEdit && !fetchingEvent && universities.length > 0 && formData.university) {
      if (!myCampusNames.includes(formData.university)) {
        alert("You don't have access to this event — it belongs to a different campus.");
        navigate('/ambassador/events');
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fetchingEvent, universities]);
  ...
  const handleExtraImagesUpload = async (e) => {
    ...
    if (filesToUpload.length === 0) { alert('Maximum 9 additional photos allowed (10 total including main)'); return; }
    try {
      ...
      for (let i = 0; i < filesToUpload.length; i++) {
        ...
        if (file.size > 10 * 1024 * 1024) { alert(`${file.name} is too large. Max 10MB per image.`); continue; }
  ...
  const stringToTimestamp = (dateString) => {
    ...
    try { return Timestamp.fromDate(new Date(dateString)); } catch { return null; }
  };
      ...
      <main className="flex-1 overflow-auto">
        <header className="bg-white border-b border-gray-200 px-4 sm:px-6 lg:px-8 py-4 sticky top-0 z-30">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => navigate('/ambassador/events')} className="p-2 hover:bg-gray-100 rounded-lg">
              <X size={24} />
            </button>
          </div>
        </header>
          ...
          <form onSubmit={handleSubmit} className="max-w-4xl mx-auto">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
              ...
              <div>
                ...
                {myCampuses.length === 0 ? (
                  <p className="text-sm text-amber-600">
                    You have no campus assigned. Ask your admin to assign you one before creating events.
                  </p>
                ) : myCampuses.length === 1 ? (
                      ...
                      <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-2 mb-3">
                        {(formData.images || []).map((img, index) => (
                          <div key={index} className="relative group">
                            <img src={img} alt={`Photo ${index + 1}`} className="w-full h-20 object-cover rounded-lg border border-gray-200" onError={(e) => { e.target.src = 'https://via.placeholder.com/80'; }} />
                            <button type="button" onClick={() => removeExtraImage(index)} className="absolute top-1 right-1 bg-red-500 hover:bg-red-600 text-white rounded-full w-5 h-5 flex items-center justify-center opacity-0 group-hover:opacity-100 transition shadow-md">
                              <X size={10} />
                            </button>
                          </div>
                        ))}
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-gray-200">
                <button type="submit" disabled={loading || uploading || uploadingExtra || myCampuses.length === 0}
                ...
                <button type="button" onClick={() => navigate('/ambassador/events')}
