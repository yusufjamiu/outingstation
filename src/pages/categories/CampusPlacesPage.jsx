...
import { collection, getDocs, doc, getDoc, updateDoc, addDoc, setDoc, deleteDoc, serverTimestamp, increment, query, where, arrayUnion, arrayRemove } from 'firebase/firestore';
  ...
  return (
    <>
      <div className="relative h-48 group" onClick={() => setShowFull(true)}>
        ...
        {images.length > 1 && (
          <>
            <button onClick={prev} className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition z-10"><ChevronLeft size={14} /></button>
            <button onClick={next} className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition z-10"><ChevronRight size={14} /></button>
          </>
        )}
        {images.length > 1 && (
      ...
      {showFull && (
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center" onClick={() => setShowFull(false)}>
          ...
          <div className="absolute top-4 left-1/2 -translate-x-1/2 text-white text-sm bg-white/20 px-3 py-1 rounded-full">{currentIndex + 1} / {images.length}</div>
          <img src={images[currentIndex]} alt={alt} className="max-w-[90vw] max-h-[85vh] object-contain rounded-lg" onClick={(e) => e.stopPropagation()} />
          {images.length > 1 && (
            <>
              <button onClick={prev} className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/30 text-white p-3 rounded-full"><ChevronLeft size={20} /></button>
              <button onClick={next} className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/30 text-white p-3 rounded-full"><ChevronRight size={20} /></button>
            </>
          )}
          {images.length > 1 && (
...
function LoginPromptModal({ vendor, action, onClose, onLogin, onSignup }) {
  ...
  const getTitle = () => {
    if (isLike) return 'Like this vendor?';
    if (isFollow) return 'Follow this university?';
    return 'Report this vendor?';
  };
  ...
  const getSubtext = () => {
    if (isLike) return 'The most liked vendors appear at the top! 🏆';
    if (isFollow) return 'Get notified only about events from universities you follow! 🎓';
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white w-full sm:max-w-sm rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden">
        ...
        <div className="p-6">
          ...
          <h3 className="text-xl font-bold text-gray-900 text-center mb-2">{getTitle()}</h3>
          <p className="text-gray-500 text-sm text-center mb-1">{getBody()}</p>
  ...
  const handleSubmit = async () => {
    ...
    try {
      await addDoc(collection(db, 'vendor_reports'), {
        vendorId: vendor.id, shopName: vendor.shopName, university: vendor.university,
        reason, details: details.trim() || null, reportedAt: serverTimestamp(), status: 'pending',
      });
      await updateDoc(doc(db, 'vendors', vendor.id), { reportCount: (vendor.reportCount || 0) + 1 });
      setSubmitted(true);
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 sticky top-0 bg-white z-10 rounded-t-3xl">
          <h3 className="font-bold text-gray-900 text-lg">{submitted ? '✅ Report Submitted' : '🚩 Report Vendor'}</h3>
          <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-full transition"><X size={18} className="text-gray-500" /></button>
        </div>
        {submitted ? (
          <div className="p-6 text-center">
            ...
            <p className="text-gray-500 text-sm mb-6">Your report about <strong>{vendor.shopName}</strong> has been submitted. Our team will review it within 24 hours.</p>
            <button onClick={onClose} className="w-full py-3 bg-cyan-500 text-white rounded-xl font-semibold hover:bg-cyan-600 transition">Done</button>
          </div>
        ) : (
          <div className="p-5">
            <div className="bg-gray-50 rounded-xl p-3 mb-4 flex items-center gap-3">
              <img src={vendor.imageUrl || 'https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?w=100'} alt={vendor.shopName}
            ...
            <div className="space-y-2 mb-4">
              {REPORT_REASONS.map((r) => (
                <button key={r.value} onClick={() => setReason(r.value)}
                  className={`w-full text-left px-4 py-3 rounded-xl text-sm font-medium transition border-2 ${reason === r.value ? 'border-red-400 bg-red-50 text-red-700' : 'border-gray-200 bg-white text-gray-700 hover:border-gray-300'}`}>
                  {r.label}
                </button>
              ))}
            </div>
            ...
            <button onClick={handleSubmit} disabled={!reason || submitting}
              ...
              {submitting ? <><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white" />Submitting...</> : '🚩 Submit Report'}
            </button>
            <p className="text-xs text-gray-400 text-center mt-3">Reports are reviewed within 24 hours. False reports may result in account restrictions.</p>
          </div>
        )}
      </div>
    </div>
...
export default function CampusPlacesPage() {
  ...
  // ✅ Like state
  const [likedVendorIds, setLikedVendorIds] = useState(new Set());
  ...
  // ✅ Toggle like — guests see login prompt
  const handleLike = async (e, vendor) => {
    e.stopPropagation();
    if (!currentUser) { setLoginPrompt({ action: 'like', vendor }); return; }
    ...
    setLocalLikeCounts(prev => ({
      ...prev,
      [vendor.id]: Math.max(0, (prev[vendor.id] || 0) + (isLiked ? -1 : 1))
    }));
    setLikingVendorId(vendor.id);

    try {
      if (isLiked) {
        await deleteDoc(doc(db, 'vendor_likes', likeDocId));
  ...
  // ✅ Report — guests see login prompt
  const handleReport = (e, vendor) => {
    e.stopPropagation();
    if (!currentUser) { setLoginPrompt({ action: 'report', vendor }); return; }
    ...
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">{selectedUniversity === 'All Universities' ? 'All universities' : selectedUniversity}</p>
        </div>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {vendorCategoriesWithCount.map((cat) => (
          <button key={cat.name} onClick={() => setSelectedVendorCategory(cat.name)}
    ...
    <div>
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            {sortedVendors.length} vendor{sortedVendors.length !== 1 ? 's' : ''} · sorted by popularity
          </p>
        </div>
      </div>
      ...
      {sortedVendors.length > 0 && (localLikeCounts[sortedVendors[0].id] || sortedVendors[0].likeCount || 0) > 0 && (
        <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5 mb-4">
          ...
          <p className="text-sm text-amber-700 font-medium">
            <strong>{sortedVendors[0].shopName}</strong> is the most popular in this category!
          </p>
        </div>
      )}
      ...
      {!currentUser && (
        <div className="flex items-center gap-3 bg-cyan-50 border border-cyan-200 rounded-xl px-4 py-3 mb-4">
          ...
          <div className="flex-1">
            <p className="text-sm text-cyan-700 font-medium">Help rank the best vendors!</p>
            <p className="text-xs text-cyan-600">Sign in to like vendors and push your favourites to the top.</p>
          </div>
          <button onClick={() => navigate('/login')}
            className="px-3 py-1.5 bg-cyan-500 text-white text-xs font-semibold rounded-lg hover:bg-cyan-600 transition flex-shrink-0">
            Sign In
          </button>
        </div>
      )}
      ...
      {sortedVendors.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="text-gray-500 text-sm">{selectedUniversity === 'All Universities' ? 'Check back soon!' : `No vendors in this category at ${selectedUniversity} yet`}</p>
          <button onClick={() => setSelectedVendorCategory(null)} className="mt-4 px-6 py-2 bg-cyan-500 text-white rounded-lg text-sm font-medium hover:bg-cyan-600 transition">Browse Other Categories</button>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {sortedVendors.map((vendor, index) => {
            ...
            const isLiked = likedVendorIds.has(vendor.id);
            const likeCount = localLikeCounts[vendor.id] ?? vendor.likeCount ?? 0;
            const isTopVendor = index < 3 && likeCount > 0;
        ...
        {!selectedVendorCategory && (
          <div className="flex gap-2 flex-wrap mb-6">
            {placeSubCategories.map((sub) => (
              <button key={sub.name} onClick={() => setSelectedSubCategory(sub.name)}
                className={`px-4 py-2 rounded-full text-sm font-medium transition ${selectedSubCategory === sub.name ? 'bg-cyan-500 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:border-cyan-400'}`}>
                {sub.icon} {sub.name}
              </button>
            ))}
          </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {filteredPlaces.map((place) => {
              ...
              return (
                <div key={place.id} onClick={() => navigate(`/event/${place.id}`)}
                  ...
                  <div className="relative" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="absolute top-3 left-3 z-10">
                      <span className="bg-purple-500 text-white text-xs px-3 py-1 rounded-full">{place.campusSubCategory || 'Campus Place'}</span>
                    </div>
                    {place.isFree && (
                  ...
                  <div className="p-4">
                    <h3 className="text-base font-bold text-gray-900 mb-2 group-hover:text-cyan-500 transition line-clamp-2">{place.title}</h3>
                    <div className="space-y-1.5 text-sm text-gray-600">
                      {place.university && <div className="flex items-center gap-2"><span>🏛️ {place.university}</span></div>}
                      <div className="flex items-center gap-2"><MapPin size={14} className="text-cyan-500" /><span>{place.location || 'Campus'}</span></div>
                      {place.openingTime && place.closingTime && <div className="flex items-center gap-2"><span>🕐 {place.openingTime} - {place.closingTime}</span></div>}
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-700 text-lg font-semibold mb-2">
              {selectedSubCategory !== 'All' ? `No ${selectedSubCategory} available at ${selectedUniversity} yet` : `No places found for ${selectedUniversity}`}
            </p>
            <p className="text-gray-500 text-sm">{selectedSubCategory !== 'All' ? 'Try selecting a different type' : 'Places will appear here once added for this campus'}</p>
            {selectedSubCategory !== 'All' && (
              <button onClick={() => setSelectedSubCategory('All')} className="mt-4 px-6 py-2 bg-cyan-500 text-white rounded-lg text-sm font-medium hover:bg-cyan-600 transition">View All Places</button>
            )}
          </div>
        )}
