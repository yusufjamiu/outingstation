...
// ✅ CHANGED — replaced the mood system (upbeat/chill/ambient) with a
// simpler Music vs Sound Effect type. Moods weren't adding much value for
// picking background audio, and this split is more useful for organizing
...
export default function AdminMusicTracks() {
  ...
  // ✅ NEW — label for a track's type, with a fallback for existing docs
  // that still have the old 'mood' field instead of 'type'
  const typeLabel = (track) => {
    const value = track.type || (track.mood ? 'music' : 'music');
    return TRACK_TYPES.find(t => t.value === value)?.label || 'Music';
  };
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      {['', 'Title', 'Type', 'Duration', 'Actions'].map(h => (
