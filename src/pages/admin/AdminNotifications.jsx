...
import { Bell, Send, Users, CheckCircle, AlertCircle, Loader, Trash2, History, Menu, RefreshCw, GraduationCap } from 'lucide-react';
...
export default function AdminNotifications() {
  ...
  const [selectedUniversity, setSelectedUniversity] = useState(''); // ✅ NEW
  const [sending, setSending] = useState(false);
  const [result, setResult] = useState(null);
  const [userCount, setUserCount] = useState(0);
  const [cities, setCities] = useState([]);
  const [universities, setUniversities] = useState([]); // ✅ NEW
  const [universityFollowerCounts, setUniversityFollowerCounts] = useState({}); // ✅ NEW
  ...
  const loadRecentNotifications = async () => {
    ...
    try {
      ...
      snapshot.docs.forEach(doc => {
        ...
        if (!grouped[key]) {
          grouped[key] = { id: doc.id, title: data.title, message: data.message, type: data.type, university: data.university, createdAt: data.createdAt, recipients: [] };
        }
        grouped[key].recipients.push({ id: doc.id, userId: data.userId });
      });
    ...
    try {
      ...
      if (targetAudience === 'city' && selectedCity) {
        ...
        return usersSnapshot.docs
          .filter(doc => (doc.data().city || '').toLowerCase().includes(targetCity))
          .map(doc => doc.id);
      }

      if (targetAudience === 'role' && selectedRole) {
        return usersSnapshot.docs
          .filter(doc => doc.data().role === selectedRole)
          .map(doc => doc.id);
      }

      // ✅ University followers
      if (targetAudience === 'university' && selectedUniversity) {
        return usersSnapshot.docs
          .filter(doc => (doc.data().followedUniversities || []).includes(selectedUniversity))
          .map(doc => doc.id);
      }
  ...
  const sendNotification = async () => {
    ...
    if (targetAudience === 'role' && !selectedRole) {
      setResult({ type: 'error', message: 'Please select a role' }); return;
    }
    if (targetAudience === 'university' && !selectedUniversity) {
      setResult({ type: 'error', message: 'Please select a university' }); return;
    }
    ...
    try {
      ...
      if (userIds.length === 0) {
        setResult({
          type: 'error',
          message: targetAudience === 'university'
            ? `No followers found for ${selectedUniversity} yet`
      ...
      const promises = userIds.map(userId =>
        addDoc(collection(db, 'notifications'), {
          ...
          type,
          university: targetAudience === 'university' ? selectedUniversity : null,
          read: false,
      ...
      setTitle(''); setMessage(''); setType('announcement');
      setTargetAudience('all'); setSelectedCity('');
      setSelectedRole(''); setSelectedUniversity('');
  ...
  const deleteNotificationBroadcast = async (notification) => {
    ...
    try {
      ...
      setResult({ type: 'error', message: `❌ Error deleting: ${error.message}` });
    }
  ...
  const notificationTypes = [
    ...
    { value: 'university_event', label: '🎓 University Event', emoji: '🎓' },
  ...
  const audienceOptions = [
    ...
    { value: 'university', label: 'University Followers', description: 'Students following a specific university' }, // ✅ NEW
    { value: 'city', label: 'Specific City', description: 'Target users in a specific city' },
    { value: 'role', label: 'By Role', description: 'Target users by role' },
  ];
        ...
        <div className="p-4 sm:p-6 lg:p-8">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
              ...
              {/* ✅ University followers stats card */}
              <div className="bg-gradient-to-r from-purple-50 to-violet-50 rounded-xl p-4 border border-purple-100">
                <div className="flex items-center gap-3">
                  <GraduationCap size={24} className="text-purple-600" />
                  <div>
                    <p className="text-sm text-gray-600">Universities Being Followed</p>
                    <p className="text-2xl font-bold text-gray-900">{Object.keys(universityFollowerCounts).length}</p>
                    <p className="text-xs text-gray-500">
                      {Object.values(universityFollowerCounts).reduce((a, b) => a + b, 0)} total follows
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-lg border border-gray-200">
                <div className="p-4 sm:p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {audienceOptions.map(option => (
                        <button key={option.value} onClick={() => {
                          ...
                          if (option.value === 'all') { setSelectedCity(''); setSelectedRole(''); setSelectedUniversity(''); }
                        }}
                          className={`p-4 rounded-lg border-2 text-left transition-all ${
                            targetAudience === option.value
                              ? option.value === 'university' ? 'border-purple-500 bg-purple-50' : 'border-cyan-500 bg-cyan-50'
                              : 'border-gray-200 hover:border-gray-300'
                          }`}>
                          <div className="font-medium text-gray-900 flex items-center gap-2">
                            {option.value === 'university' && <GraduationCap size={16} className="text-purple-500" />}
                            {option.label}
                          </div>
                  ...
                  {targetAudience === 'university' && (
                    <div>
                      <label className="block text-sm font-semibold text-gray-900 mb-2">Select University</label>
                      <select value={selectedUniversity} onChange={(e) => setSelectedUniversity(e.target.value)}
                        className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                        <option value="">Choose a university...</option>
                        {universities.map(uni => (
                          <option key={uni} value={uni}>
                            {uni} ({universityFollowerCounts[uni] || 0} followers)
                          </option>
                        ))}
                      </select>
                      {selectedUniversity && (
                        <p className="text-sm text-purple-600 font-medium mt-2">
                          🎓 {universityFollowerCounts[selectedUniversity] || 0} student{(universityFollowerCounts[selectedUniversity] || 0) !== 1 ? 's' : ''} will receive this notification
                        </p>
                      )}
                    </div>
                  )}
                  ...
                  {/* Notification Type */}
                  <div>
                    <label className="block text-sm font-semibold text-gray-900 mb-2">Notification Type</label>
                    <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                      {notificationTypes.map(notifType => (
                        <button key={notifType.value} onClick={() => setType(notifType.value)}
                          className={`p-3 rounded-lg border-2 text-center transition-all ${type === notifType.value ? 'border-cyan-500 bg-cyan-50 text-cyan-700' : 'border-gray-200 text-gray-700 hover:border-gray-300'}`}>
                  ...
                  <div>
                    ...
                    <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
                      placeholder={targetAudience === 'university' && selectedUniversity ? `🎓 New Event at ${selectedUniversity}!` : '🎉 New Feature Launched!'}
                      maxLength={100} />
                  ...
                  <div>
                    ...
                    <textarea value={message} onChange={(e) => setMessage(e.target.value)}
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
                      rows={4}
                      placeholder={targetAudience === 'university' && selectedUniversity ? `There's a new campus event at ${selectedUniversity}. Check it out!` : 'Check out our new event filters and enhanced search!'}
                      maxLength={500} />
            ...
            <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4 sm:p-6">
              ...
              <ul className="space-y-2 text-sm text-gray-700">
                <li>• 🎓 Use <strong>University Followers</strong> for campus events — students get relevant notifications only</li>
