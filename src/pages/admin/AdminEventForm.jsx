...
export default function AdminEventForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    subCategory: 'events', // ✅ always 'events' — places use AdminPlaceForm
    ...
    price: '', isFree: false, capacity: '', imageUrl: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const eventData = {
        ...formData,
        subCategory: 'events', // ✅ always events
              ...
              <div>
                ...
                <div className="flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-cyan-400 transition">
                  <div className="space-y-1 text-center">
                    {formData.imageUrl ? (
                      <div>
                        <img src={formData.imageUrl} alt="Preview" className="mx-auto h-48 w-auto rounded-lg" />
                        <button type="button" onClick={() => setFormData(prev => ({ ...prev, imageUrl: '' }))} className="mt-2 text-sm text-red-600 hover:text-red-700">
                          Remove Image
                        </button>
                      </div>
                    ) : (
