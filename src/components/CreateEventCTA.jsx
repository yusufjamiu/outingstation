...
export default function CreateEventCTA() {
  return (
    <section className="bg-[#5ADAEE] py-16 md:py-24 px-4 md:px-6 relative overflow-hidden">
      <div className="absolute top-0 left-0 w-96 h-96 bg-white rounded-full opacity-20 blur-3xl"></div>
              ...
              <React.Fragment key={s.label}>
                <div className="flex flex-col items-center gap-2">
                  <div className="w-14 h-14 rounded-2xl bg-white border border-white flex items-center justify-center shadow-sm">
                    <Icon size={22} className="text-[#5ADAEE]" />
                  </div>
                  <span className="text-xs text-gray-800 font-medium">{s.label}</span>
                </div>
                {i < STOPS.length - 1 && (
                  <ArrowRight size={16} className="text-gray-900/30 hidden sm:block" />
                )}
              </React.Fragment>
            );
          })}
